...
public class RdsStack extends Stack {
    ...
    public RdsStack(final Construct scope, final String id, final StackProps props, Vpc vpc) {
        ...
        .create(this, "databasePassword") // Cria um parâmetro de senha para o banco de dados
        ...
        .fromSecurityGroupId(this, id, vpc.getVpcDefaultSecurityGroup()); // Recupera o Security Group padrão da VPC pelo ID
        securityGroup.addIngressRule(Peer.anyIpv4(), Port.tcp(3306)); // Permite conexões TCP na porta 3306 (MySQL) de qualquer IP
        ...
        DatabaseInstance databaseInstance = DatabaseInstance.Builder
                ...
                .instanceIdentifier("aws-project01-db") // Define o identificador da instância do banco de dados
                ...
                .credentials(
                        Credentials.fromUsername("Admin",
                                CredentialsFromUsernameOptions.builder()
                                        .password(SecretValue.unsafePlainText(dataBasePassword.getValueAsString())) // Define a senha do usuário Admin
                                        .build())) // Finaliza as credenciais do banco de dados
                .instanceType(InstanceType.of(InstanceClass.BURSTABLE2, InstanceSize.MICRO)) // Define o tipo de
                                                                                             // instância
                .multiAz(false) // Define se a instância é Multi-AZ
                .allocatedStorage(10) // Define o armazenamento alocado em GB
                .securityGroups(Collections.singletonList(securityGroup)) // Associa o Security Group à instância do banco
                                                                      // de dados
        ...
        CfnOutput.Builder.create(this, "rds-endpoint")
                ...
                .value(databaseInstance.getDbInstanceEndpointAddress()) // Exporta o endpoint da instância do banco de dados
                .build(); // Finaliza a construção do output

        CfnOutput.Builder.create(this, "rds-password")
                .exportName("rds-password")
                .value(dataBasePassword.getValueAsString()) // Exporta a senha do banco de dados
                .build(); // Finaliza a construção do output
    }
